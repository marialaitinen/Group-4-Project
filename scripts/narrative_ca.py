...
try:
    stop_words_list = stopwords.words('catalan')
except:
    stop_words_list = []

# Load Input (Updated to use sample_*.csv)
input_path = os.path.join(data_dir, "sample_ca.csv")
...
print(f"Training BERTopic on {len(docs)} Catalan articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['protesta', 'manifestació', 'vaga'], ['govern', 'política'], ['democràcia', 'reforma'], ['internacional', 'global'], ['religió', 'església'], ['eleccions', 'vot'], ['economia', 'preus'], ["cop d'estat", 'militar'], ['violència', 'guerra']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_ca.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_ca.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_ca.csv")) 
print(f"--- FINAL VAR INPUT READY (CA) ---")
