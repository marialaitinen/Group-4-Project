...
# NOTE: Update these timestamps for Kyrgyz specific protest events!
protest_starts = [
...
if all_windows:
    ...
    ax.set_title(f"Average Grievance Trajectory - Kyrgyz ({len(all_windows)} Windows)")
    ax.invert_xaxis() 
    ax.legend(bbox_to_anchor=(1.01, 1), loc='upper left', fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(data_dir, "avg_preprotest_trajectory_ky.png"), dpi=150)
    plt.close()
...
if records:
    ...
    output_path = os.path.join(data_dir, "lagged_correlations_ky.csv")
