...
# Load the Tamil data
input_path = os.path.join(data_dir, "var_input_ta.csv")
...
print("Data differenced. Modeling daily 'shocks' for Tamil data.")
...
# NOTE: Update these timestamps for Tamil specific protest events!
protest_starts = [
...
if all_windows:
    ...
    ax.set_title(f"Average Grievance Trajectory - Tamil ({len(all_windows)} Windows)")
    ax.invert_xaxis() 
    ax.legend(bbox_to_anchor=(1.01, 1), loc='upper left', fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(data_dir, "avg_preprotest_trajectory_ta.png"), dpi=150)
    plt.close()
...
for lag in lags_to_test:
    ...
    print(f"\n========== TAMIL GRANGER CAUSALITY RESULTS ({lag}-DAY LAG) ==========")
...
if records:
    ...
    output_path = os.path.join(data_dir, "lagged_correlations_ta.csv")
