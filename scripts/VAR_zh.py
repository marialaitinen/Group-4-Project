...
# FEEDBACK #3: Chinese protest events
protest_starts_all = [
...
for start in protest_starts:
    # FIX: Create a continuous daily index so 1 row exactly = 1 day (handles missing weekends)
...
if all_windows:
    ...
    ax_traj.set_title(f"Average Grievance Trajectory Across {len(protest_starts)} Pre-Protest Windows (ZH)", fontsize=16, pad=15)
    ...
    plt.close(fig_traj)
    print(f"Saved Category 1: {traj_path}")
...
for start in protest_starts:
    # FIX: Continuous calendar index ensures accurate lagging and prevents empty binary arrays
    ...
    for grievance in grievance_columns:
        ...
        for lag in range(-POST_DAYS, PRE_DAYS + 1):
            ...
            if len(aligned) > 2:
                # FIX: Check for zero variance to prevent math errors returning NaN
...
if not results_df.empty:
    fig_events, axes_events = plt.subplots(1, n_events, figsize=(5 * n_events, 6), sharey=True, layout='constrained', squeeze=False)
