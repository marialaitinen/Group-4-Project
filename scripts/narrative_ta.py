...
try:
    stop_words_list = stopwords.words('tamil')
except:
    stop_words_list = []

# Load Input (Updated to use sample_*.csv)
input_path = os.path.join(data_dir, "sample_ta.csv")
...
print(f"Training BERTopic on {len(docs)} Tamil articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['எதிர்ப்பு', 'ஆர்ப்பாட்டம்', 'வேலைநிறுத்தம்'], ['அரசாங்கம்', 'அரசியல்'], ['ஜனநாயகம்', 'சீர்திருத்தம்'], ['சர்வதேச', 'உலகளாவிய'], ['மதம்', 'கோயில்'], ['தேர்தல்', 'வாக்கு'], ['பொருளாதார', 'விலைகள்'], ['சதி', 'இராணுவ'], ['வன்முறை', 'போர்']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_ta.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_ta.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_ta.csv")) 
print(f"--- FINAL VAR INPUT READY (TA) ---")
