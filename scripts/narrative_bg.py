...
try:
    stop_words_list = stopwords.words('bulgarian')
except:
    stop_words_list = []

# Load Input (Updated to use sample_*.csv)
input_path = os.path.join(data_dir, "sample_bg.csv")
...
print(f"Training BERTopic on {len(docs)} Bulgarian articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['протест', 'митинг', 'стачка'], ['правителство', 'политика'], ['демокрация', 'реформа'], ['международен', 'глобален'], ['религия', 'църква'], ['избори', 'вот'], ['икономика', 'цени'], ['преврат', 'военен'], ['насилие', 'война']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_bg.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_bg.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_bg.csv")) 
print(f"--- FINAL VAR INPUT READY (BG) ---")
