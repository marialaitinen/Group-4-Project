...
# Vietnamese protest events (Unchanged)
protest_starts_all = [
...
if all_windows:
    ...
    ax_traj.set_title(f"Average Grievance Trajectory Across {len(protest_starts)} Pre-Protest Windows (VI)", fontsize=16, pad=15)
    ...
    try:
        ...
        for grievance in grievance_columns:
            if grievance in df_diff.columns:
                ...
                print(f"{clean_name.ljust(15)} -> PROTESTS : p-value = {p_val:.4f} | {significant}")
...
if not results_df.empty:
    ...
    if len(axes_events) > 0:
        axes_events[0].legend(bbox_to_anchor=(1.05, 1.0), loc='upper left', fontsize=11)
