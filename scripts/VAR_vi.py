...
# Load the Vietnamese data
input_path = os.path.join(data_dir, "var_input_vi.csv")
...
print("Data differenced. Modeling daily shocks for Vietnamese data.")
...
# Vietnamese protest events (PRESERVED)
protest_starts_all = [
    pd.Timestamp("2018-06-09")
]
...
if all_windows:
    ...
    ax_traj.set_title(f"Aggregated Average Grievance Trajectory Across {len(protest_starts)} Pre-Protest Windows (VI)", fontsize=16, pad=15)
    ax_traj.invert_xaxis()
    ax_traj.legend(bbox_to_anchor=(1.01, 1), loc='upper left', fontsize=11)
    
    traj_path = os.path.join(data_dir, "trajectory_vi.png")
...
if all_windows:
    ...
    fig_indiv.suptitle("Individual Grievance Trajectories Per Protest Event (VI)", 
    ...
    indiv_traj_path = os.path.join(data_dir, "trajectory_individual_vi.png")
...
for lag in lags_to_test:
    try:
        ...
        print(f"\n========== VIETNAMESE GRANGER CAUSALITY RESULTS ({lag}-DAY LAG) ==========")
...
if not granger_df.empty:
    granger_df.to_csv(os.path.join(data_dir, "granger_results_vi.csv"), index=False)
else:
...
if not results_df.empty:
    ...
    results_df.to_csv(os.path.join(data_dir, "lagged_correlations_vi.csv"), index=False)
...
def plot_heatmap_on_ax(avg_df, lag_range, title_suffix, ax):
    ...
    ax.set_title(
        f"Lagged Correlation - Protests (vi) | {title_suffix}\n"
...
if not results_df.empty:
    ...
    fig_hm.suptitle("Pearson Lagged Correlation Heatmaps (VI)", fontsize=20, fontweight='bold', y=1.02)
    ...
    hm_path = os.path.join(data_dir, "heatmaps_vi.png")
...
if not results_df.empty:
    ...
    fig_events.suptitle("Per-Event Correlation Breakdown (VI)", fontsize=20, fontweight='bold', y=1.05)
    ...
    events_path = os.path.join(data_dir, "per_event_lines_vi.png")
...
if irf_records:
    ...
    fig_irf.suptitle("Impulse Response Functions: Shock to Protest Outcome (VI)",
                      fontsize=18, fontweight='bold', y=1.02)
    ...
    irf_path = os.path.join(data_dir, "irf_vi.png")
...
if coeff_records:
    ...
    fig_hm2.suptitle("VAR Coefficients: Grievances to Protest Outcome (VI)",
                      fontsize=18, fontweight='bold', y=1.02)
    ...
    coeff_path = os.path.join(data_dir, "coeff_heatmap_vi.png")
