...
try:
    stop_words_list = stopwords.words('hungarian')
except:
    stop_words_list = []

# Load Input (Updated to use sample_*.csv)
input_path = os.path.join(data_dir, "sample_hu.csv")
...
print(f"Training BERTopic on {len(docs)} Hungarian articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['tüntetés', 'demonstráció', 'sztrájk'], ['kormány', 'politika'], ['demokrácia', 'reform'], ['nemzetközi', 'globális'], ['vallás', 'egyház'], ['választás', 'szavazás'], ['gazdaság', 'árak'], ['puccs', 'katonai'], ['erőszak', 'háború']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_hu.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_hu.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_hu.csv")) 
print(f"--- FINAL VAR INPUT READY (HU) ---")
