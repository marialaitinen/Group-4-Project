...
input_path = os.path.join(data_dir, "sample_it.csv")
...
print(f"Training BERTopic on {len(docs)} Italian articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['protesta', 'dimostrazione', 'sciopero'], ['governo', 'politica'], ['democrazia', 'riforma'], ['internazionale', 'globale'], ['religione', 'chiesa'], ['elezione', 'voto'], ['economia', 'prezzi'], ['colpo di stato', 'militare'], ['violenza', 'guerra']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_it.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_it.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_it.csv")) 
print(f"--- FINAL VAR INPUT READY (IT) ---")
