...
try:
    stop_words_list = stopwords.words('greek')
except:
    stop_words_list = []

# Load Input (Updated to use sample_*.csv)
input_path = os.path.join(data_dir, "sample_el.csv")
...
print(f"Training BERTopic on {len(docs)} Greek articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['διαμαρτυρία', 'διαδήλωση', 'απεργία'], ['κυβέρνηση', 'πολιτική'], ['δημοκρατία', 'μεταρρύθμιση'], ['διεθνής', 'παγκόσμιος'], ['θρησκεία', 'εκκλησία'], ['εκλογές', 'ψήφος'], ['οικονομία', 'τιμές'], ['πραξικόπημα', 'στρατιωτικός'], ['βία', 'πόλεμος']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_el.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_el.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_el.csv")) 
print(f"--- FINAL VAR INPUT READY (EL) ---")
