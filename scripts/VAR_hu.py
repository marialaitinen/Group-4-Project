...
# Load the Hungarian data
input_path = os.path.join(data_dir, "var_input_hu.csv")
...
print("Data differenced. Modeling daily shocks for Hungarian data.")
...
# Hungarian protest events (PRESERVED EXPLICITLY)
protest_starts_all = [
...
if all_windows:
    ...
    ax_traj.set_title(f"Aggregated Average Grievance Trajectory Across {len(protest_starts)} Pre-Protest Windows (HU)", fontsize=16, pad=15)
    ax_traj.invert_xaxis()
    ax_traj.legend(bbox_to_anchor=(1.01, 1), loc='upper left', fontsize=11)
    
    traj_path = os.path.join(data_dir, "trajectory_hu.png")
...
if all_windows:
    ...
    fig_indiv.suptitle("Individual Grievance Trajectories Per Protest Event (HU)", 
    ...
    indiv_traj_path = os.path.join(data_dir, "trajectory_individual_hu.png")
...
for lag in lags_to_test:
    try:
        ...
        print(f"\n========== HUNGARIAN GRANGER CAUSALITY RESULTS ({lag}-DAY LAG) ==========")
...
if not granger_df.empty:
    granger_df.to_csv(os.path.join(data_dir, "granger_results_hu.csv"), index=False)
else:
...
if not results_df.empty:
    ...
    results_df.to_csv(os.path.join(data_dir, "lagged_correlations_hu.csv"), index=False)
...
def plot_heatmap_on_ax(avg_df, lag_range, title_suffix, ax):
    ...
    ax.set_title(
        f"Lagged Correlation - Protests (hu) | {title_suffix}\n"
...
if not results_df.empty:
    ...
    fig_hm.suptitle("Pearson Lagged Correlation Heatmaps (HU)", fontsize=20, fontweight='bold', y=1.02)
    ...
    hm_path = os.path.join(data_dir, "heatmaps_hu.png")
...
if not results_df.empty:
    ...
    fig_events.suptitle("Per-Event Correlation Breakdown (HU)", fontsize=20, fontweight='bold', y=1.05)
    ...
    events_path = os.path.join(data_dir, "per_event_lines_hu.png")
...
if irf_records:
    ...
    fig_irf.suptitle("Impulse Response Functions: Shock to Protest Outcome (HU)",
                      fontsize=18, fontweight='bold', y=1.02)
    ...
    irf_path = os.path.join(data_dir, "irf_hu.png")
...
if coeff_records:
    ...
    fig_hm2.suptitle("VAR Coefficients: Grievances to Protest Outcome (HU)",
                      fontsize=18, fontweight='bold', y=1.02)
    ...
    coeff_path = os.path.join(data_dir, "coeff_heatmap_hu.png")
