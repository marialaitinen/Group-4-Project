...
try:
    stop_words_list = stopwords.words('vietnamese')
except:
    stop_words_list = []

# Load Input (Updated to use sample_*.csv)
input_path = os.path.join(data_dir, "sample_vi.csv")
...
print(f"Training BERTopic on {len(docs)} Vietnamese articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['biểu tình', 'biểu dương', 'đình công'], ['chính phủ', 'chính trị'], ['dân chủ', 'cải cách'], ['quốc tế', 'toàn cầu'], ['tôn giáo', 'nhà thờ'], ['bầu cử', 'bỏ phiếu'], ['kinh tế', 'giá cả'], ['đảo chính', 'quân sự'], ['bạo lực', 'chiến tranh']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_vi.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_vi.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_vi.csv")) 
print(f"--- FINAL VAR INPUT READY (VI) ---")
