...
try:
    stop_words_list = stopwords.words('kyrgyz')
except:
    stop_words_list = []

# Load Input (Updated to use sample_*.csv)
input_path = os.path.join(data_dir, "sample_ky.csv")
...
print(f"Training BERTopic on {len(docs)} Kyrgyz articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['митинг', 'нааразычылык', 'стачка'], ['өкмөт', 'саясат'], ['демократия', 'реформа'], ['эл аралык', 'глобалдык'], ['дин', 'мечит'], ['шайлоо', 'добуш'], ['экономика', 'баалар'], ['төңкөрүш', 'аскердик'], ['зомбулук', 'согуш']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_ky.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_ky.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_ky.csv")) 
print(f"--- FINAL VAR INPUT READY (KY) ---")
