...
if all_windows:
    ...
    ax.set_title(f"Average Grievance Trajectory - Bengali ({len(all_windows)} Windows)")
    ax.invert_xaxis() 
    ax.legend(bbox_to_anchor=(1.01, 1), loc='upper left', fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(data_dir, "avg_preprotest_trajectory_bn.png"), dpi=150)
    plt.close()
...
if records:
    ...
    output_path = os.path.join(data_dir, "lagged_correlations_bn.csv")
