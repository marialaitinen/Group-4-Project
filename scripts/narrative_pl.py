...
try:
    stop_words_list = stopwords.words('polish')
except:
    stop_words_list = []

# Load Input (Updated to use sample_*.csv)
input_path = os.path.join(data_dir, "sample_pl.csv")
...
print(f"Training BERTopic on {len(docs)} Polish articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['protest', 'demonstracja', 'strajk'], ['rząd', 'polityka'], ['demokracja', 'reforma'], ['międzynarodowy', 'globalny'], ['religia', 'kościół'], ['wyborze', 'głosowanie'], ['ekonomia', 'ceny'], ['pucz', 'wojskowy'], ['przemoc', 'wojna']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_pl.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_pl.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_pl.csv")) 
print(f"--- FINAL VAR INPUT READY (PL) ---")
