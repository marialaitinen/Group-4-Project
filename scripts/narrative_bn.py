...
try:
    stop_words_list = stopwords.words('bengali')
except:
    stop_words_list = []

# Load Input (Updated to use sample_*.csv)
input_path = os.path.join(data_dir, "sample_bn.csv")
...
print(f"Training BERTopic on {len(docs)} Bengali articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['বিক্ষোভ', 'সমাবেশ', 'ধর্মঘট'], ['সরকার', 'রাজনীতি'], ['গণতন্ত্র', 'সংস্কার'], ['আন্তর্জাতিক', 'বিশ্বব্যাপী'], ['ধর্ম', 'মসজিদ'], ['নির্বাচন', 'ভোট'], ['অর্থনীতি', 'দাম'], ['অভ্যুত্থান', 'সামরিক'], ['সহিংসতা', 'যুদ্ধ']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_bn.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_bn.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_bn.csv")) 
print(f"--- FINAL VAR INPUT READY (BN) ---")
