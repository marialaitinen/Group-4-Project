...
# Load the Czech data
input_path = os.path.join(data_dir, "var_input_cs.csv")
...
print("Data differenced. Modeling daily shocks for Czech data.")
...
# Czech protest events (PRESERVED EXPLICITLY)
protest_starts_all = [
...
if all_windows:
    ...
    ax_traj.set_title(f"Aggregated Average Grievance Trajectory Across {len(protest_starts)} Pre-Protest Windows (CS)", fontsize=16, pad=15)
    ax_traj.invert_xaxis()
    ax_traj.legend(bbox_to_anchor=(1.01, 1), loc='upper left', fontsize=11)
    
    traj_path = os.path.join(data_dir, "trajectory_cs.png")
...
if all_windows:
    ...
    fig_indiv.suptitle("Individual Grievance Trajectories Per Protest Event (CS)", 
    ...
    indiv_traj_path = os.path.join(data_dir, "trajectory_individual_cs.png")
...
for lag in lags_to_test:
    try:
        ...
        print(f"\n========== CZECH GRANGER CAUSALITY RESULTS ({lag}-DAY LAG) ==========")
...
if not granger_df.empty:
    granger_df.to_csv(os.path.join(data_dir, "granger_results_cs.csv"), index=False)
else:
...
if not results_df.empty:
    ...
    results_df.to_csv(os.path.join(data_dir, "lagged_correlations_cs.csv"), index=False)
...
def plot_heatmap_on_ax(avg_df, lag_range, title_suffix, ax):
    ...
    ax.set_title(
        f"Lagged Correlation - Protests (cs) | {title_suffix}\n"
...
if not results_df.empty:
    ...
    fig_hm.suptitle("Pearson Lagged Correlation Heatmaps (CS)", fontsize=20, fontweight='bold', y=1.02)
    ...
    hm_path = os.path.join(data_dir, "heatmaps_cs.png")
...
if not results_df.empty:
    ...
    fig_events.suptitle("Per-Event Correlation Breakdown (CS)", fontsize=20, fontweight='bold', y=1.05)
    ...
    events_path = os.path.join(data_dir, "per_event_lines_cs.png")
...
if irf_records:
    ...
    fig_irf.suptitle("Impulse Response Functions: Shock to Protest Outcome (CS)",
                      fontsize=18, fontweight='bold', y=1.02)
    ...
    irf_path = os.path.join(data_dir, "irf_cs.png")
...
if coeff_records:
    ...
    fig_hm2.suptitle("VAR Coefficients: Grievances to Protest Outcome (CS)",
                      fontsize=18, fontweight='bold', y=1.02)
    ...
    coeff_path = os.path.join(data_dir, "coeff_heatmap_cs.png")
