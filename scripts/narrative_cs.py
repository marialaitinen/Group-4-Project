...
try:
    stop_words_list = stopwords.words('czech')
except:
    stop_words_list = []

# Load Input (Updated to use sample_*.csv)
input_path = os.path.join(data_dir, "sample_cs.csv")
...
print(f"Training BERTopic on {len(docs)} Czech articles...")

# Run BERTopic with Translated Seeds
seed_topic_list = [['protest', 'demonstrace', 'stávka'], ['vláda', 'politika'], ['demokracie', 'reforma'], ['mezinárodní', 'globální'], ['náboženství', 'církev'], ['volby', 'hlasování'], ['ekonomika', 'ceny'], ['převrat', 'vojenský'], ['nasilí', 'válka']]
...
topics_over_time.to_csv(os.path.join(data_dir, "narrative_pulse_data_cs.csv"))
df_sample.to_csv(os.path.join(data_dir, "classified_articles_cs.csv"), index=False)
...
daily_ts.to_csv(os.path.join(data_dir, "var_input_cs.csv")) 
print(f"--- FINAL VAR INPUT READY (CS) ---")
