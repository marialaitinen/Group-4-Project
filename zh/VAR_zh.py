...
data_dir = script_dir  # zh data lives alongside this script
os.makedirs(data_dir, exist_ok=True)

input_path = os.path.join(data_dir, "zh_var_input_ready.csv")
...
# Chinese data has: narrative_protest (outcome), narrative_corruption, narrative_economic
grievance_columns = ['narrative_corruption', 'narrative_economic']
protest_col = 'narrative_protest'
...
if all_windows:
    ...
    ax.set_title(f"Average Grievance Trajectory Across {len(protest_starts)} Pre-Protest Windows (ZH)")
    ax.invert_xaxis()
    ax.legend()
    plt.tight_layout()
    ...
    try:
        ...
        for grievance in grievance_columns:
            if grievance in df_diff.columns:
                test_result = results.test_causality(protest_col, [grievance], kind='f')
...
for start in protest_starts:
    ...
    ][grievance_columns + [protest_col]].copy()
